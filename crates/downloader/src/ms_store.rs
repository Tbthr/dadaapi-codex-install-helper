...
async fn resolve_remote_msix_url(architecture: CpuArchitecture) -> Result<Url, MsStoreError> {
    ...
    let manifest = parse_remote_manifest(&body)?;
...
fn parse_remote_manifest(body: &[u8]) -> Result<RemoteManifest, MsStoreError> {
    ...
    if release.body.matches(REMOTE_MANIFEST_BEGIN).count() != 1
        || release.body.matches(REMOTE_MANIFEST_END).count() != 1
    {
        return Err(MsStoreError::InvalidMetadata);
    }
    let (_, after_begin) = release
        .body
        .split_once(REMOTE_MANIFEST_BEGIN)
        .ok_or(MsStoreError::InvalidMetadata)?;
    let (manifest, _) = after_begin
...
mod remote_manifest_tests {
    ...
    #[test]
    fn extracts_metadata_from_the_final_release_body() {
        let body = br#"{
          "tag_name":"v1.0.0",
          "prerelease":false,
          "body":"release notes\n\n<!-- DADAAPI_MSIX_LINKS_V1\n{\"generatedAt\":\"2026-08-03T00:00:00Z\",\"packages\":{\"arm64\":{\"url\":\"https://dl.delivery.mp.microsoft.com/arm64\",\"expiresAt\":\"2026-08-04T00:00:00Z\"},\"x64\":{\"url\":\"https://dl.delivery.mp.microsoft.com/x64\",\"expiresAt\":\"2026-08-04T00:00:00Z\"}}}\nDADAAPI_MSIX_LINKS_END -->"
        }"#;
        let manifest = parse_remote_manifest(body).expect("valid release metadata");
        assert_eq!(
            manifest.packages.x64.url,
            "https://dl.delivery.mp.microsoft.com/x64"
        );
    }

    #[test]
    fn rejects_prerelease_or_ambiguous_release_metadata() {
        let prerelease = br#"{"tag_name":"v1.0.0","prerelease":true,"body":"<!-- DADAAPI_MSIX_LINKS_V1\n{}\nDADAAPI_MSIX_LINKS_END -->"}"#;
        assert!(parse_remote_manifest(prerelease).is_err());
