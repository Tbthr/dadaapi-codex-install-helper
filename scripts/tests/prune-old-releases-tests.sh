...
[
  {"id": 10, "tag_name": "v1.0.10", "draft": false, "prerelease": false, "assets": [
    {"name": "Dada-Assistant_1.0.10_x64-setup.exe"},
    {"name": "Dada-Assistant_1.0.10_arm64-setup.exe"},
    {"name": "Dada-Assistant_1.0.10_universal.dmg"},
    {"name": "checksums.txt"}
  ]},
  {"id": 9, "tag_name": "v1.0.9", "draft": true, "prerelease": false, "assets": []}
]
EOF
  cat > "$MOCK_GITEE_RELEASES" <<'EOF'
[
  {"id": 110, "tag_name": "v1.0.10", "prerelease": false, "assets": [
    {"name": "Dada-Assistant_1.0.10_x64-setup.exe"},
    {"name": "Dada-Assistant_1.0.10_arm64-setup.exe"},
    {"name": "Dada-Assistant_1.0.10_universal.dmg"},
    {"name": "checksums.txt"},
  ...
  {"id": 109, "tag_name": "v1.0.9", "prerelease": true, "assets": []}
]
EOF
}
...
jq -e 'length == 1 and .[0].tag_name == "v1.0.10"' "$MOCK_GITHUB_RELEASES" >/dev/null
jq -e 'length == 1 and .[0].tag_name == "v1.0.10"' "$MOCK_GITEE_RELEASES" >/dev/null

bash "$repository_root/scripts/release/prune-old-releases.sh" >/dev/null

write_fixture
jq 'map(if .tag_name == "v1.0.10" then .prerelease = true else . end)' \
