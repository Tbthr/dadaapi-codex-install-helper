...
下列入口脚本固定来自不可变的 `v1.0.10` 标签；脚本默认安装两端最新正式版本。Windows 命令使用系统 `curl.exe` 限制 HTTPS 和重定向，将脚本完整下载到当前用户临时目录，确认真实退出码并按严格 UTF-8 读取到内存后执行，最后清理：

Windows（Gitee 默认入口，严格 Gitee 优先）：

```powershell
$u = "https://gitee.com/lyq_power/dadaapi-codex-install-helper/raw/v1.0.10/scripts/install.ps1"
...
curl -fsSL --proto '=https' --proto-redir '=https' --max-redirs 5 --connect-timeout 10 --max-time 60 \
  ...
  https://gitee.com/lyq_power/dadaapi-codex-install-helper/raw/v1.0.10/scripts/install.sh \
...
```powershell
$u = "https://raw.githubusercontent.com/Tbthr/dadaapi-codex-install-helper/v1.0.10/scripts/install.ps1"
$d = Join-Path ([IO.Path]::GetTempPath()) ("DadaBootstrap-" + [Guid]::NewGuid().ToString("N"))
$p = Join-Path $d "install.ps1"
...
curl -fsSL --proto '=https' --proto-redir '=https' --max-redirs 5 --connect-timeout 10 --max-time 60 \
  --max-filesize 1048576 \
  https://raw.githubusercontent.com/Tbthr/dadaapi-codex-install-helper/v1.0.10/scripts/install.sh \
  -o "$installer_file" || exit $?
DADA_ASSISTANT_INSTALL_SOURCE=github /bin/sh "$installer_file"
```

安装器稳定参数为 `DADA_ASSISTANT_INSTALL_VERSION=latest|vN.N.N` 与 `DADA_ASSISTANT_INSTALL_SOURCE=auto|gitee|github`。例如在执行下载后的脚本前设置 `DADA_ASSISTANT_INSTALL_VERSION=v1.0.10`，即可固定安装当前正式版本 `v1.0.10`。`v1.0.0` 至 `v1.0.9` 仅保留不可变标签。每次两源 `latest` 与资产验证通过后，发布流水线会删除旧 Release 记录并保留全部历史标签，使 GitHub/Gitee Release 页面始终只展示最新正式版本。脚本只安装并启动哒哒助手，不写入中文配置或系统代理。
