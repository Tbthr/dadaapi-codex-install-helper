...
- 路由构建密钥只允许存在于无审批、仅接受受保护 `main` 与 `v*` 标签的 `production-release` GitHub Environment。`GITEE_TOKEN` 作为仓库 Secret 只供发布、镜像或明确的元数据任务读取，普通 PR 和 CI 不得引用。
- `v*` 正式标签必须指向当前受保护 `main` 提交；发布后标签和资产均不可更新或删除。
- GitHub 与 Gitee 都必须在服务端保护 `v*` 标签，禁止删除或更新；发布脚本的“不覆盖”检查不能替代仓库级保护规则。
- Microsoft Store 回退元数据只能从固定 Gitee Final Release 的公开 API 读取，必须拒绝重定向、错误标签、Prerelease、重复标记和非 Microsoft 官方下载主机。定时任务只更新 Release 说明，不得上传第五个正式资产。
