...
生产桌面客户端不依赖 SSH 后台、配置服务器或订阅中继。独立 `wocao-hub-routes` 仓库通过 GitHub Actions 定时拉取上游订阅，并向 GitHub Raw 发布三个静态文件：

- `manifest.json`
- `routes.sig`
- `routes.enc`

发布端使用 XChaCha20-Poly1305 加密订阅正文，使用 Ed25519 签名清单原始字节，并在清单中使用 SHA-256 绑定密文。客户端严格按“HTTPS 下载 → 清单验签 → 字段与有效期验证 → 密文大小和哈希校验 → AEAD 解密”的顺序处理。
