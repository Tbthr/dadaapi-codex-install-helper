...
本文档用于首个正式 Release `v1.0.3` 及后续补丁版本。`v1.0.0` 与 `v1.0.1` 仅保留不可变源码标签；`v1.0.2` 未通过远程安装门禁，仅保留为 Prerelease。发行包不使用 Apple Developer ID、公证或 Windows Authenticode，安装命令通过用户目录安装和系统下载区块移除实现无管理员密码安装。
